...
const TurnTimer = ({ timeLeft, isMyTurn, turnDuration }) => {
  ...
  // 👉 Freeze timer visually when it's NOT your turn
  const displayTime = infinite
    ? '∞'
    : isMyTurn
      ? timeLeft
      : turnDuration

  // 👉 Freeze progress ring when it's NOT your turn
  const progress = infinite
    ? 1
    : (isMyTurn ? timeLeft : turnDuration) / (turnDuration || 30)

  const dashOffset = CIRCUMFERENCE * (1 - progress)

  const isUrgent = !infinite && isMyTurn && timeLeft <= 10
